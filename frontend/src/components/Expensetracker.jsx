...
const Expensetracker = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const res = await fetch("http://13.61.187.150:5000/api/expenses");
  ...
  const addExpense = async (newExpense) => {
    try {
      const response = await fetch("http://13.61.187.150:5000/api/expenses", {
        method: "POST",
        headers: {
  ...
  const deleteExpense = async (id) => {
    try {
      const res = await fetch(`http://13.61.187.150:5000/api/expenses/${id}`, {
        method: "DELETE"
      });
  ...
  const clearAllExpenses = async () => {
    if (window.confirm("Are you sure you want to clear all expenses?")) {
      for (let expense of expenseList) {
        await fetch(`http://13.61.187.150:5000/api/expenses/${expense._id}`, {
          method: "DELETE"
        });
      }
      setExpenseList([]);
