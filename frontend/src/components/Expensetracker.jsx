...
const Expensetracker = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const res = await fetch("http://13.51.64.174:5000/api/expenses");
  ...
  const deleteExpense = async (id) => {
    try {
      const res = await fetch(`http://13.51.64.174:5000/api/expenses/${id}`, {
        method: "DELETE"
      });
  ...
  const clearAllExpenses = async () => {
    if (window.confirm("Are you sure you want to clear all expenses?")) {
      for (let expense of expenseList) {
        await fetch(`http://13.51.64.174:5000/api/expenses/${expense._id}`, {
          method: "DELETE"
        });
      }
      setExpenseList([]);
